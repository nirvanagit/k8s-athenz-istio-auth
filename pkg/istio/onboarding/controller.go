...
func (c *Controller) callbackHandler(err error, item *processor.Item) error {
	...
	if c.queue.NumRequeues(queueKey) >= queueNumRetries {
		log.Errorf("%s Max number of retries reached for %s.", logPrefix, queueKey)
		return nil
	}
	if item != nil {
		log.Infof("%s Retrying operation %s on %s due to processing error for %s", logPrefix, item.Operation, item.Resource.Key(), queueKey)
	}
